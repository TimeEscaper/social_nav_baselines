...
def obj(z, current_target):
    ...
    z = [v, w, x, y, theta]
    current_target = goal point that is to be headed for
    """

    return ((z[2] - current_target[0])**2 + (z[3] - current_target[1])**2) ** 0.5

    return casadi.norm_2(z[2:4] - current_target[:2])

    return (  1000 * (z[2] - current_target[0])**2  # Penalty on x position
            + 1000 * (z[3] - current_target[1])**2  # Penalty on y position
            + z[0]**2    # Penalty on v control input
            + z[1]**2 )  # Penalty on w control input
...
def generate_pathplanner(gen_solver):
    ...
    model.xinitidx = range(2,5) # use this to specify on which variables initial conditions
    # are imposed
    ...
    if gen_solver:
        shutil.rmtree("/home/aleksandr/Research/social_nav_baselines/FORCESNLPsolver", ignore_errors=True, onerror=None)
        solver = model.generate_solver(options=codeoptions)
    else:
        solver = forcespro.nlp.Solver.from_directory("/home/aleksandr/Research/social_nav_baselines/FORCESNLPsolver")
    return model, solver
...
def plot_data(x, init_state, goal, x_pred_stacked):
    ...
    def animate(i) -> None:
        ...
        ax.plot(x_pred_stacked[cnt, 1, :], x_pred_stacked[cnt, 0, :], linewidth=1, color='blue', linestyle='dashed')
    ...
    anim = FuncAnimation(fig, animate, frames=frames, interval=400, repeat=False)
    anim.save("/home/aleksandr/Research/social_nav_baselines/test_forces/test.gif", 'pillow', frames)
...
def plot_obj(data):
    ...
    plt.savefig("/home/aleksandr/Research/social_nav_baselines/test_forces/cost.png")

def main():
    gen_solver = True # if true, than generate new solver, else use existed
    ...
    init_state = np.array([5, 5, np.pi*0.25])
    ...
    for k in range(sim_length):
        ...
        # Set goal position
        problem["all_parameters"] = np.tile(goal_point, (model.N,))
