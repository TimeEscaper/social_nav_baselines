...
def generate_circular_scenarios():
    ...
    for total_peds in [1, 2, 4, 7]:
        for scenario_id in range(30):
            ...
            circle_rad = np.random.uniform(min_circle_rad, max_circle_rad)
            robot_initial_position = np.array([circle_rad * np.cos(phis[-1]), circle_rad * np.sin(phis[-1]), 0])
            ...
            with open(fr'evaluation/scenarios/circular_crossing/{total_peds}/{scenario_id}.yaml', 'w') as outfile:
...
def generate_random_scenarios():
    ...
    for total_peds in [1, 2, 4, 7]:
        for scenario_id in range(30):
            ...
            with open(fr'evaluation/scenarios/random/{total_peds}/{scenario_id}.yaml', 'w') as outfile:
                yaml.dump(config, outfile)
...
def generate_parallel_scenarios():
    ...
    for total_peds in [1, 2, 4, 7]:
        for scenario_id in range(30):
            ...
            # add robot position
            robot_initial_position = np.array([X_coordinates[-1], np.random.uniform(-4, -1), 0])
            ...
            with open(fr'evaluation/scenarios/parallel_traffic/{total_peds}/{scenario_id}.yaml', 'w') as outfile:
                yaml.dump(config, outfile)
...
def generate_perpendicular_scenarios():
    ...
    for total_peds in [1, 2, 4, 7]:
        for scenario_id in range(30):
            ...
            with open(fr'evaluation/scenarios/perpendicular_traffic/{total_peds}/{scenario_id}.yaml', 'w') as outfile:
                yaml.dump(config, outfile)
