...
def add_box(ax, ped_amount, q25, q75, color, width=0.1):
    x0 = ped_amount - width / 2
    y0 = q25
    height = q75 - q25
    ax.add_patch(Rectangle((x0, y0), width, height, facecolor=color, alpha=0.2))
    ...
    for scene_id, scene in enumerate(scenes_list):
        ...
        for controller_id, controller in enumerate(controller_list):
            ...
            for i, ped in enumerate(pedestrian_range):
                ...
                add_box(axs_graph[0], ped, q25, q75, color=DEFAULT_COLOR_HEX_PALETTE[controller_id])
            ...
            # Collisions
            axs_graph[1].plot(pedestrian_range, mean_tables_data[scene_id, controller_id, len(pedestrian_range):len(pedestrian_range)*2], linewidth=3, label=f"{controller}",  marker="o", markersize=SIZE//2, color=DEFAULT_COLOR_HEX_PALETTE[controller_id])
            ...
            # Timeouts
            axs_graph[2].plot(pedestrian_range, mean_tables_data[scene_id, controller_id, len(pedestrian_range)*2:], linewidth=3, label=f"{controller}", marker="o", markersize=SIZE//2, color=DEFAULT_COLOR_HEX_PALETTE[controller_id])
