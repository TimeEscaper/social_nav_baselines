...
def batched_constant_velocity_model(positions, current_velocity, num_future_positions=12, dt=0.1):
    # dt = 0.1  # time step, can be adjusted as needed
    ...
    for i in range(num_future_positions):
        future_positions[:, i, 0] = positions[:, -1, 0] + current_velocity[:, 0] * dt * (i+1)
        future_positions[:, i, 1] = positions[:, -1, 1] + current_velocity[:, 1] * dt * (i+1)
