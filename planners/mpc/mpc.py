...
class DoMPCController:
    def __init__(self, dt: float, 
        ...
        self.ub_cons = (self._r_rob + self._r_ped) ** 2 + self._min_safe_dist ** 2
        self.ub_cons_vect = np.array([self.ub_cons for _ in range(self._total_peds)])
        # inequality constrain for pedestrian
        self.xy_rob = casadi.hcat([self._model._x.cat[:2] for _ in range(self._total_peds)])
        self.xy_peds = self.p_peds[:2, :]
        self.sq = (self.xy_rob - self.xy_peds) ** 2
        peds_dist = self.sq[0, :] + self.sq[1, :]
        self._mpc.set_nl_cons('dist_to_peds', -peds_dist, ub=-self.ub_cons_vect)
...
def main() -> None:
    ...
    with open(rf'{config_path}/{file_name}_config.yaml') as f:
        ...
        col_rgb: List[Tuple[int]] = [graphic_tools.hex_to_rgb(col) for col in col_hex]
    ...
    while running:
        ...
        if hold_time >= dt: # Every ten-th step of simulation we calculate control
            ...
            for k in range(n_horizon + 1):
                if k == 0:
                    controller.mpc_ped_tvp_fun['_tvp', 0, 'p_peds'] = gt_ped_data.T
                else: 
                    controller.mpc_ped_tvp_fun['_tvp', k, 'p_peds'] = controller.propagate_all_pedestrians(controller.mpc_ped_tvp_fun['_tvp', k-1, 'p_peds'], dt, total_peds)
            # render pedestrian predictions
            for i in range(total_peds):
                for k in range(0, n_horizon + 1):
                    pred_ped__xy_at_step_k = np.array(controller.mpc_ped_tvp_fun['_tvp', k, 'p_peds']).T[i][:2]
                    renderer.draw(f"ped_{i}_{k}", CircleDrawing(pred_ped__xy_at_step_k, 0.05, col_rgb[i], 0))
            ...
            for i in range(n_horizon+1):
                renderer.draw(f"{1000+i}", CircleDrawing(array_xy_pred[i], 0.03, (255, 100, 0), 0))
            hold_time = 0.
        # Simulate
        sim.step(u_pred)
