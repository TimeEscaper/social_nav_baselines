...
def dwa_make_step(X_rob_cur: np.ndarray,
    ...
    v_range = np.arange(config['lb'][0], config['ub'][0], config['v_resolution'])
    if 0 not in v_range: np.append(v_range, 0)
    w_range = np.arange(config['lb'][1], config['ub'][1], config['w_resolution'])
    if 0 not in w_range: np.append(w_range, 0)
...
def get_pred_rob_traj(X_rob_cur, v, w, n_horizon, dt) -> np.ndarray:
    ...
    for i in range(1, n_horizon):
        new_x = pred_trajectory[i - 1, 0] + v * np.cos(pred_trajectory[i - 1, 2]) * dt
        new_y = pred_trajectory[i - 1, 1] + v * np.sin(pred_trajectory[i - 1, 2]) * dt
...
def heading(pred_rob_traj, p_rob_ref, n_horizon) -> float:
    ...
    for pred_rob_pos in pred_rob_traj:
        ...
        overall_deviation_angle = (overall_deviation_angle + np.pi) % (2 * np.pi) - np.pi
        cost = abs(overall_deviation_angle) / np.pi # TODO: Это я типа нормализацию придумал подобно статье
...
def main() -> None:
    ...
    pred_peds_traj = np.zeros([int(simulation_time/dt), total_peds, n_horizon, 2 ])
    ...
    while running:
        ...
        if hold_time >= dt: # Every ten-th step of simulation we calculate control
            ...
            cur_dist_to_goal = math.sqrt( (X_rob_cur[0] - p_rob_ref[0]) ** 2 + (X_rob_cur[1] - p_rob_ref[1]) ** 2 )
            ...
            undetect_ped_ind = list(set(range(total_peds)) - set(detect_ped_ind))
            gt_ped_data = np.concatenate([sim.current_state.world.pedestrians.poses[:, :2], sim.current_state.world.pedestrians.velocities[:, :2]], axis=1)
            gt_ped_data[undetect_ped_ind] = x_dummy_ped
            gt_peds_traj[sim_step] = sim.current_state.world.pedestrians.poses[:, :2].flatten() # save for matplotlib
            ...
            for k in range(n_horizon):
                if k == 0:
                    ...
                    propagation_ped_traj[k, :, :] = propagate_all_pedestrians(propagation_ped_traj[k-1, :, :], dt)
            # render pedestrian predictions
            for k in range(0, n_horizon):
                for i in range(total_peds):
                    renderer.draw(f"ped_{i}_{k}", CircleDrawing(propagation_ped_traj[k, i, :2], 0.05, col_rgb[i], 0))
                    pred_peds_traj[sim_step, i, k, :] = propagation_ped_traj[k, i, :2]
            
            # Calculate action        
            u_pred, rob_traj = dwa_make_step(X_rob_cur, propagation_ped_traj, p_rob_ref, n_horizon, dt, config)
            pred_rob_traj[sim_step, :] = rob_traj[:, :2]
            sim_step += 1
            #print(pred_rob_traj)
            for i in range(n_horizon):
                renderer.draw(f"{1000+i}", CircleDrawing(rob_traj[i, :2], 0.03, (255, 100, 0), 0))
            hold_time = 0.
