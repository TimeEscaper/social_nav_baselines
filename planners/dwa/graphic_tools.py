...
def make_animation(y_rob_plt: np.ndarray,
    ...
    def plot_pedestrian(x_ped_plt, y_ped_plt, cnt, i) -> None:
        ...
        ax.plot(x_ped_plt[:cnt], y_ped_plt[:cnt], linewidth=3,
                color=col_hex[i], label=f'Pedestrian {i+1}')
        # plot pedestrian i area
        ped1_radius_plot = plt.Circle(
            (x_ped_plt[cnt], y_ped_plt[cnt]), r_ped, fill=False, linewidth=5, color=col_hex[i])
        ...
        ax.plot(pred_peds_traj[cnt, i, :, 1], pred_peds_traj[cnt, i,
                :, 0], color=col_hex[i], linestyle='dashed', linewidth=1)
    ...
    def animate(i) -> None:
        ...
        # annotate robot goal
        goal_coord = (round(float(p_rob_ref[0]), 2), round(
            float(p_rob_ref[1]), 2))
        ax.annotate(f'Goal: {goal_coord}', goal_coord -
                    np.array([0, r_rob]) - annotation_offset,  ha='center')
        # plot robot start
        ax.plot(float(X_rob_init[0]), float(
            X_rob_init[1]), 'yo', markersize=10)
        # annotate robot start
        start_coord = (round(float(X_rob_init[0]), 2), round(
            float(X_rob_init[1]), 2))
        ...
        ax.plot(pred_rob_traj[cnt, :, 1], pred_rob_traj[cnt,
    ...
    dirs = [i for i in os.listdir(results_path) if os.path.isdir(
        os.path.join(results_path, i)) and result_name in i]
    if dirs:
    ...
    anim.save(
        rf'{result_directory}/{result_name}{ind+1}{animation_format}', 'ffmpeg', 10)
    shutil.copy(rf'{config_path}/{file_name}_config.yaml',
                rf'{result_directory}/{file_name}_config_{ind+1}.yaml')
    time_end = time.time()
    print(
        f'Animation: Finished! Elapsed time: {round(time_end - time_start, 2)}s')
...
def mpc_step_response(mpc_data,
    ...
    ax[0].plot(time_plt, x_plt, color='#1f77b4',
               linestyle='solid', label=r'$x, [m]$')
    ax[0].plot(time_plt, y_plt, color='#ff7f0e',
               linestyle='solid', label=r'$y, [m]$')
    ax[0].plot(time_plt, phi_plt, color='#2ca02c',
               linestyle='solid', label=r'$\phi, [rad]$')
    ax[1].plot(time_plt, v_plt, color='#d62728',
               linestyle='solid', label=r'$v, [\frac{m}{s}]$')
    ax[1].plot(time_plt, w_plt, color='#9467bd', linestyle='solid',
               label=r'$\omega, [\frac{rad}{s^2}]$')
    ax[2].plot(time_plt, u_a_plt, color='#1f77b4',
               linestyle='solid', label=r'$u_a, [\frac{m}{s^2}]$')
    ax[2].plot(time_plt, u_alpha_plt, color='#ff7f0e',
               linestyle='solid', label=r'$\omega, [\frac{rad}{s^2}]$')
