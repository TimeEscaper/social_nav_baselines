from core.controllers import FORCESPROMPCController
...
DEFAULT_SCENE_CONFIG_PATH = r"evaluation/scenes/circular_crossing/0/0.yaml"
DEFAULT_CONTROLLER_CONFIG_PATH = r"evaluation/controllers/FORCES-MPC.yaml"
...
def main(scene_config_path: str = DEFAULT_SCENE_CONFIG_PATH,
    ...
    controller = FORCESPROMPCController(np.array(config["init_state"]),
        ...
        if hold_time >= controller.dt:
            ...
            if error >= config["tolerance_error"]:
               ...
               #                                                  simulator.current_state.world.pedestrians.velocities[:, :2]], axis=1)
                ...
                control = controller.make_step(state, None)
    ...
    #visualizer.make_animation(f"FORCESPRO MPC", 
    #                          result_path, 
    #                          config)
